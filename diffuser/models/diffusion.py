...
class GaussianInvDynDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        ...
        # self.register_buffer('sqrt_recip_alphas_1malphas_cumprod', torch.sqrt(1. / ((1. - alphas_cumprod) * alphas_cumprod)))
        # self.register_buffer('sqrt_recip_1malphas_cumprod', torch.sqrt(1. / (1. - alphas_cumprod)))
    ...
    def predict_start_from_noise2(self, x_t, t, noise):
        '''
            if self.predict_epsilon, model output is (scaled) noise;
            otherwise, model predicts x0 directly
        '''
    ...
    def predict_noise_from_start(self, x_t, t, x0):
        return (
            (1. / extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)) * x_t -
            (1. / extract(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape)) * x0
        )
    ...
    def p_mean_variance(self, x, cond, t, returns=None):
        if type(returns) == list:
            x0 = self.model(x, cond, t, returns[0], force_dropout=True)
            epsilon = self.predict_noise_from_start(x, t, x0)
            for ret in returns:
                x0_cond = self.model(x, cond, t, ret, use_dropout=False)
                x0_uncond = self.model(x, cond, t, ret, force_dropout=True)
                eps_cond = self.predict_noise_from_start(x, t, x0_cond)
                eps_uncond = self.predict_noise_from_start(x, t, x0_uncond)
                epsilon += (self.condition_guidance_w) * (eps_cond - eps_uncond)
        else:
            if self.returns_condition:
                # epsilon could be epsilon or x0 itself
                epsilon_cond = self.model(x, cond, t, returns, use_dropout=False)
        ...
        if type(returns) == list:
            x_recon = self.predict_start_from_noise2(x, t=t, noise=epsilon)
        else:
            x_recon = self.predict_start_from_noise(x, t=t, noise=epsilon)
    ...
    def p_sample_(self, x, cond, t, returns=None):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, cond=cond, t=t, returns=returns)
        noise = 0.5*torch.randn_like(x)
        # no noise when t == 0
    ...
    @torch.no_grad()
    def p_sample_loop_slice(self, xt, cond, start, returns=None):
        batch_size = len(xt)
        device = self.betas.device
        ...
        for i in range(start, end - 1, -1):
            timesteps = torch.full((batch_size,), i, device=device).long()
            xt = self.p_sample(xt, cond, timesteps, returns)
    ...
    @torch.no_grad()
    def p_sample_loop_acc(self, shape, cond, returns=None, verbose=False, return_diffusion=False):
        device = self.betas.device
    ...
    @torch.no_grad()
    def conditional_sample_acc(self, cond, returns=None, horizon=None, *args, **kwargs):
        device = self.betas.device
        batch_size = len(cond[0])
        horizon = horizon or self.horizon
        shape = (batch_size, horizon, self.observation_dim)
        return self.p_sample_loop_acc(shape, cond, returns, *args, **kwargs)

    @torch.no_grad()
    def p_warm_sample_loop(self, shape, cond, warm_sample, warm_diffusion_steps, returns=None, verbose=True, return_diffusion=False):
        device = self.betas.device

        batch_size = shape[0]

        warm_sample = apply_conditioning(warm_sample, cond, 0)

        t = torch.full((batch_size,), fill_value=warm_diffusion_steps, device=device, dtype=torch.long)

        x_noisy = self.q_sample(x_start=warm_sample, t=t)
        x = apply_conditioning(x_noisy, cond, 0)

        if return_diffusion: diffusion = [x]

        progress = utils.Progress(warm_diffusion_steps) if verbose else utils.Silent()
        for i in reversed(range(0, warm_diffusion_steps)):
            timesteps = torch.full((batch_size,), i, device=device, dtype=torch.long)
            x = self.p_sample(x, cond, timesteps, returns)
            x = apply_conditioning(x, cond, 0)
